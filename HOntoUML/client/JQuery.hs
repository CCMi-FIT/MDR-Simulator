...
documentJq :: IO JQuery
documentJq = ffi "(function () { return $(document); })"
...
getHeight = ffi "(function (jq) { return jq.innerHeight(); })"

setHeight :: Int -> JQuery -> IO JQuery
setHeight = ffi "(function (val, jq) { jq.innerHeight(val); return jq; })"
...
isComplete :: JQuery -> IO Bool
isComplete = ffi "(function (jq) { console.log(jq.get(0)); console.log(jq.get(0).complete); return jq.get(0).complete; })"
...
onMouseDown :: Handler -> JQuery -> IO JQuery
onMouseDown ev jq = do
  ffiMouseDown ev jq
  return jq
  where
    ffiMouseDown :: Handler -> JQuery -> IO ()
    ffiMouseDown = ffi "(function (ev, jq) { jq.mousedown(ev); })"

onMouseUp :: Handler -> JQuery -> IO JQuery
onMouseUp ev jq = do
  ffiMouseUp ev jq
  return jq
  where
    ffiMouseUp :: Handler -> JQuery -> IO ()
    ffiMouseUp = ffi "(function (ev, jq) { jq.mouseup(ev); })"

onMouseMove :: Handler -> JQuery -> IO JQuery
onMouseMove ev jq = do
  ffiMouseMove ev jq
  return jq
  where
    ffiMouseMove :: Handler -> JQuery -> IO ()
    ffiMouseMove = ffi "(function (ev, jq) { jq.mousemove(ev); })"
...
load :: JQuery -> IO JQuery
load = ffi "(function (jq) { jq.load(); return jq; })"
...
getEvClientX :: JSAny -> IO Int
getEvClientX = ffi "(function (ev) { return ev.clientX; })"
...
selectSVG selector jq = let selectorJs = toJSString selector in doFFI selectorJs jq
  where
    doFFI :: JSString -> JQuery -> IO JQuery
    doFFI = ffi "(function (selector, jq) { if (jq[0].contentDocument !== null) { var res = $(selector, jq[0].contentDocument.documentElement); if (res.length === 0) { console.warn('empty $ selection ' + selector); }; return res; } else {console.error('empty contentDocument'); return jq;} })"
